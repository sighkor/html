<?php if(!defined("__XE__"))exit;?><script src="/modules/marketplace/tpl/js/marketplace_admin.js"></script>
<script src="/modules/marketplace/tpl/modules/module/tpl/js/module_admin.js"></script>
<script>
</script>
<?php Context::addJsFile("modules/marketplace/ruleset/insertMarketplaceForBasic.xml", FALSE, "", 0, "body", TRUE, "") ?><form  id="fo_obj" class="x_form-horizontal" action="./" method="post" enctype="multipart/form-data"><input type="hidden" name="error_return_url" value="<?php echo htmlspecialchars(getRequestUriByServerEnviroment(), ENT_COMPAT | ENT_HTML401, 'UTF-8', false) ?>" /><input type="hidden" name="mid" value="<?php echo $__Context->mid ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" /><input type="hidden" name="ruleset" value="insertMarketplaceForBasic" />
	<input type="hidden" name="act" value="procMarketplaceAdminUpdateMarketplaceFroBasic" />
	<input type="hidden" name="marketplace_name" value="<?php echo $__Context->module_info->mid ?>" />
