...
<table id="marketplaceList" class="x_table x_table-striped x_table-hover">
	<caption>
	...
	<tbody>
		<?php if($__Context->marketplace_list&&count($__Context->marketplace_list))foreach($__Context->marketplace_list as $__Context->no=>$__Context->val){ ?><tr>
...
echo $__Context->module_category[$__Context->val->module_category_srl]->title;
			...
			<td>
				<a href="<?php echo getUrl('act','dispMarketplaceAdminMarketplaceInfo','module_srl',$__Context->val->module_srl) ?>" class="x_icon-cog" title="<?php echo $__Context->lang->cmd_setup ?>"><?php echo $__Context->lang->cmd_setup ?></a>
				...
				<a href="<?php echo getUrl('act','dispMarketplaceAdminDeleteMarketplace','module_srl', $__Context->val->module_srl) ?>" class="x_icon-remove" title="<?php echo $__Context->lang->cmd_delete ?>"><?php echo $__Context->lang->cmd_delete ?></a>
			</td>
		...
		<?php if(!$__Context->marketplace_list){ ?><tr>
			<td><?php echo $__Context->lang->no_marketplace_instance ?></td>
		</tr><?php } ?>
	</tbody>
</table>
...
if(!in_array($__Context->key, array('mid', 'vid', 'act'))){ ?><input type="hidden" name="<?php echo $__Context->key ?>" value="<?php echo $__Context->val ?>" /><?php }} ?>
	...
	<span class="x_pull-right x_btn-group">
		<!--<a class="x_btn x_btn-inverse" href="<?php echo getUrl('act','dispMarketplaceAdminInsertMarketplace','module_srl','') ?>"><?php echo $__Context->lang->cmd_create_marketplace ?></a>-->
		<a class="x_btn modalAnchor _manage_selected" href="#manageSelectedModule"><?php echo $__Context->lang->cmd_manage_selected_marketplace ?></a>
	</span>
</div>
...
jQuery(function($){
	...
	$('a.modalAnchor._manage_selected').click(function(){
		if($docTable.find('tbody :checked').length == 0){
			...
			alert('<?php echo $__Context->lang->choose_marketplace_instance ?>');
			return false;
		}
	});
});
